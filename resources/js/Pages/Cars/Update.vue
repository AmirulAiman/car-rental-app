...
const {brands, properties, statuses, rental_type, car} = defineProps(['brands','properties','statuses','rental_type', 'car']);
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Update Your Car Detail</h2>
        </template>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid gap-1 grid-cols-8">
                                <div class="relative flex items-start py-4 ml-2" v-for="property in properties" :key="property.id">
                                    ...
                                    <label :for="property.id" class="inline-flex items-center justify-between w-auto p-2 font-medium tracking-tight border rounded-lg cursor-pointer bg-brand-light text-brand-black border-gray-500 peer-checked:border-gray-400 peer-checked:bg-gray-700 peer-checked:text-white peer-checked:font-semibold peer-checked:decoration-brand-dark decoration-2">
