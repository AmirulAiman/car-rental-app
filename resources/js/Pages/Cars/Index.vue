...
const {cars, brands, isAdmin} = defineProps(['cars','brands']);

const carList = computed(() => {
    if(search.value){
        return cars.filter((c) => c.name.include(search.value))
    }
    if(filter.value){
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-4 gap-2" v-if="carList.length > 0">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg px-3 py-1 hover:scale-105 transition-all duration-300" v-for="car in carList" :key="car.id">
                        <div class="p-6 text-gray-900 text-center font-bold">{{ car.name }}</div>
                        <div class="flex justify-between">
                            <div class="px-2 text-gray-900 rounded-md bg-gray-300 w-1/2">
                                <h1 class="font-semibold text-center">Deposit</h1>
                                <h1 class="text-center">RM {{ car.deposit }}</h1>
                            </div>
                            <div class="px-2 text-gray-900 rounded-md bg-gray-300 w-fill">
                                <h1 class="font-semibold text-center">Rental Charge</h1>
                                <h1 class="text-center">RM {{ car.rental_charge }}/Daily</h1>
                            </div>
                        </div>
                        <div class="flex justify-end p-3">
                            <a :href="'#'" class="rounded px-3 py-1 bg-blue-800 text-blue-500 hover:bg-blue-500 hover:text-blue-800 mr-2" v-if="$page.props.auth.user.role == 'customer'">
